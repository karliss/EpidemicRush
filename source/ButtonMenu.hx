...
class ButtonMenu extends FlxTypedGroup<FlxSprite>
{
    ...
    public function updateInput(input:MoveState) {
        if (delay <= 0) {
            var pressed:Bool = false;
            if (buttons.length > 0) {
                if (input.moveVec.y >= 0.5 || (input.moveVec.y >= -0.01 && input.moveVec.x >= 0.5)) {
                    selected += 1;
                    pressed = true;
                } else if (input.moveVec.y <= -0.5 || (input.moveVec.y <= 0.01 && input.moveVec.x <= -0.5)) {
                    selected -= 1;
                    pressed = true;
                }
                if (selected < 0) {
